...
@[      elif field.type.is_array]@
            self._@(field.name) = kwargs.get('@(field.name)', [])
@[      elif field.type.type == 'byte']@
            self._@(field.name) = kwargs.get('@(field.name)', bytes([0]))
@[      elif field.type.type == 'char']@
            self._@(field.name) = kwargs.get('@(field.name)', chr(0))
@[      else]@
            self._@(field.name) = kwargs.get('@(field.name)', @(get_python_type(field.type))())
@[      end if]@
@[    end if]@
@[  end for]@
        else:
@[  for field in spec.fields]@
@[    if field.default_value]@
            self._@(field.name) = @(spec.base_type.type).@(field.name.upper())__DEFAULT
@[    else]@
@[      if field.type.array_size and not field.type.is_upper_bound]@
@[        if field.type.type == 'byte']@
            self._@(field.name) = [bytes([0]) for x in range(@(field.type.array_size))]

@[        elif field.type.type == 'char']@
            self._@(field.name) = [chr(0) for x in range(@(field.type.array_size))]
@[        else]@
            self._@(field.name) = [@(get_python_type(field.type))() for x in range(@(field.type.array_size))]
@[        end if]@
@[      elif field.type.is_array]@
            self._@(field.name) = []
@[      elif field.type.type == 'byte']@
            self._@(field.name) = bytes([0])
@[      elif field.type.type == 'char']@
            self._@(field.name) =  chr(0)
